...
sample = sample[sample['PM25'] < 200]

plt.scatter(sample['WSPM'], sample['PM25'], alpha=0.3, color='green', label='Observed')

# Plot regression line
wspm_range = np.linspace(0, 8, 100)
plt.plot(wspm_range, 110.4 + (-17.70) * wspm_range, color='steelblue', linewidth=2, label='OLS fit')

plt.xlabel('WSPM (m/s)')
plt.ylabel('PM2.5 (μg/m³)')
plt.title('OLS Regression — Wind Speed vs PM2.5')
plt.xlim(0, 8)
plt.ylim(0, 300)
plt.legend()
plt.show()
...
print(f'  {"AIC":<25} {aic_ols:>8.1f} {aic_math:>8.1f}')
print(f'  {"BIC":<25} {bic_ols:>8.1f} {bic_math:>8.1f}')
print(f'  {"R²":<25} {r2_ols:>8.4f} {r2_math:>8.4f}')


# %% [markdown]
# - ΔAIC = −21.5: Even after penalising for the extra parameter, the Math model wins strongly.
...
# AIC/BIC bar chart
x = np.arange(2)
axes[0].bar(x-0.18, [aic_ols, bic_ols],   0.3, label='OLS',  color='steelblue', alpha=0.85)
axes[0].bar(x+0.18, [aic_math, bic_math],  0.3, label='Math', color='red',       alpha=0.85)
...
# 
# - β1 represents the Average Treatment Effect (ATE): the expected change in PM2.5 for a 1 m/s increase in wind speed.
# 
# - Higher wind speed disperses particulate matter, reducing PM2.5.
# 
# - The confounding bias is large (−17.65 μg/m³), indicating that TEMP strongly 
# confounds the WSPM–PM2.5 relationship. Without adjustment, the naive model over-estimates the effect of wind speed. After adjustment, the effect is near zero and non-significant (p = 0.51), suggesting the raw correlation was almost entirely driven by confounding.
